...
What REVIEW.md's work plan asked of phase 5: `mmbedit` with a
configurable compiler command, `cc -r prog.bas` end to end, relocatable
paths in `ccbc`; gate, a pty harness driving `mmbedit` and the manual's
...
sibling `bcrun`, `mmbedit`'s F2 runs the sibling `cc`, and the object
...
F2 in `mmbedit` is "save, exit, compile and run": the editor execs the
compiler with `-r` and the file. On the board that is `/usr/bin/cc`; on
a PC it is the `cc` beside the editor. `MMBEDIT_CC` now names another -
...
mmbedit_pty.py` is one now, and it follows the manual's walkthrough
(`mmbedit prog.bas`, then F2) through a real pseudo-terminal, 80 by 40,
the editor's controlling tty:

1. `mmbedit hello.bas` on an empty file. The harness waits for the status
   line's `F1:Save`, types `Print "hello from mmbedit"`, presses F1 as
   ...
   holds the line.
2. `mmbedit hello.bas` again, then F2 (`ESC O Q`). The editor saves,
   prints `cc -r hello.bas`, execs the compiler named by `MMBEDIT_CC` -
   this build's `cc` - which translates, compiles and runs the program,
   and `hello from mmbedit` arrives on the same terminal after the
...
* The book's walkthrough is followed by a script, not by a reader; the
  manual's `mmbedit` chapter matches what the script sees, and that is
  as far as a machine can take it.
* `mmbedit` still asks the display for the console mode through
  `pc3_sys_open()`; with no server and `PC3_AUTOSTART` at its default a
  bare `mmbedit` on a PC starts one. That is the intended shape for a
